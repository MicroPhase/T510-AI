...
module t510_ai_iq_capture(
    ...
    assign capture_clear = mode_exit | radio_rst;
    ...
    always @(posedge radio_clk) begin
        ...
        if (radio_rst) begin
            ...
            if (rx_sync_timestamp_strobe) begin
                sync_target_time <= rx_sync_timestamp;
                sync_wait <= 1'b1;
            end

            if (stream_start && capture_mode == STREAM_MODE)
                stream_enable <= 1'b1;

            if (capture_one_block && capture_mode == PACKET_MODE)
            ...
            if (cpack_valid) begin
                ...
                if (!active_chunk && !active_packet_open) begin
